...
int main(int argc, char **argv) {
    ...
    using Cache_ = nv_gpu_cache::gpu_cache<key_type, ref_counter_type, empty_key, SET_ASSOCIATIVITY, SLAB_SIZE>; 
...
#pragma omp parallel default(none) shared(h_keys, cache, h_emb_table, increase, embedding_vec_size, query_length, emb_size, iter_round) num_threads(num_threads)
{
    ...
    *******************************************************************************/
    ...
    h_keys = (key_type*)malloc(SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type));
    h_vals = (float*)malloc(SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float));
    float* h_new_vals = (float*)malloc(SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float));
    ...
    // Generating random unique keys
    cache_key_gen.fill_unique(h_keys, SLAB_SIZE * SET_ASSOCIATIVITY, cache_capacity_in_set, empty_key);
    // Filling values vector according to the keys
    fill_vec(h_keys, h_vals, embedding_vec_size, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set, increase);
    // Set new value
    increase = 1.0f;
    // Filling values vector according to the keys
    fill_vec(h_keys, h_new_vals, embedding_vec_size, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set, increase);
    ...
    CUDA_CHECK(cudaHostAlloc((void**)&h_insert_keys, SLAB_SIZE * cache_capacity_in_set * sizeof(key_type), cudaHostAllocPortable));
    CUDA_CHECK(cudaHostAlloc((void**)&h_insert_vals, SLAB_SIZE * cache_capacity_in_set * embedding_vec_size * sizeof(float), cudaHostAllocPortable));
    CUDA_CHECK(cudaHostAlloc((void**)&h_dump_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type), cudaHostAllocPortable));
    CUDA_CHECK(cudaHostAlloc((void**)&h_vals_retrieved, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float), cudaHostAllocPortable));
    CUDA_CHECK(cudaHostAlloc((void**)&h_acc_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type), cudaHostAllocPortable));

    CUDA_CHECK(cudaMalloc((void**)&d_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type)));
    CUDA_CHECK(cudaMalloc((void**)&d_vals, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float)));
    CUDA_CHECK(cudaMalloc((void**)&d_insert_keys, SLAB_SIZE * cache_capacity_in_set * sizeof(key_type)));
    CUDA_CHECK(cudaMalloc((void**)&d_insert_vals, SLAB_SIZE * cache_capacity_in_set * embedding_vec_size * sizeof(float)));
    CUDA_CHECK(cudaMalloc((void**)&d_dump_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type)));
    CUDA_CHECK(cudaMalloc((void**)&d_vals_retrieved, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float)));
    CUDA_CHECK(cudaMalloc((void**)&d_dump_counter, sizeof(size_t)));
    CUDA_CHECK(cudaMalloc((void**)&d_missing_index, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(uint64_t)));
    CUDA_CHECK(cudaMalloc((void**)&d_missing_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type)));
    CUDA_CHECK(cudaMalloc((void**)&d_missing_len, sizeof(size_t)));
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_keys, h_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * sizeof(key_type), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(d_vals, h_new_vals, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set * embedding_vec_size * sizeof(float), cudaMemcpyHostToDevice, stream));
    // Wait for stream to complete
    CUDA_CHECK(cudaStreamSynchronize(stream));
    ...
    for(size_t i = 0; i < SET_ASSOCIATIVITY; i++){
        ...
        memcpy(h_acc_keys + SLAB_SIZE * cache_capacity_in_set * i, h_insert_keys, SLAB_SIZE * cache_capacity_in_set * sizeof(key_type));

        // Copy the <k,v> pairs from host to device
        CUDA_CHECK(cudaMemcpyAsync(d_insert_keys, h_insert_keys, SLAB_SIZE * cache_capacity_in_set * sizeof(key_type), cudaMemcpyHostToDevice, stream));
        CUDA_CHECK(cudaMemcpyAsync(d_insert_vals, h_insert_vals, SLAB_SIZE * cache_capacity_in_set * embedding_vec_size * sizeof(float), cudaMemcpyHostToDevice, stream));
        ...
        cache -> Update(d_keys, SLAB_SIZE * SET_ASSOCIATIVITY * cache_capacity_in_set, d_vals, stream, SLAB_SIZE);
        // Wait for stream to complete 
        CUDA_CHECK(cudaStreamSynchronize(stream));
        // Elapsed wall time
        time_b = W_time() - time_a;
        ...
        CUDA_CHECK(cudaMemcpyAsync(&h_dump_counter, d_dump_counter, sizeof(size_t), cudaMemcpyDeviceToHost, stream));
        // Wait for stream to complete
        CUDA_CHECK(cudaStreamSynchronize(stream));
        // Check the dump counter
        assert(h_dump_counter == SLAB_SIZE * cache_capacity_in_set * (i + 1));
        // Query all the dumped keys from the cache
        cache -> Query(d_dump_keys, h_dump_counter, d_vals_retrieved, d_missing_index, d_missing_keys, d_missing_len, stream);
        // Copy result from device to host
        CUDA_CHECK(cudaMemcpyAsync(h_dump_keys, d_dump_keys, h_dump_counter * sizeof(key_type), cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(h_vals_retrieved, d_vals_retrieved, h_dump_counter * embedding_vec_size * sizeof(float), cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaMemcpyAsync(&h_missing_len, d_missing_len, sizeof(size_t), cudaMemcpyDeviceToHost, stream));
        // Wait for stream to complete
        CUDA_CHECK(cudaStreamSynchronize(stream));
        // Check result
